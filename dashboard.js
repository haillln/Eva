...
function wireEquityControls() {
  ...
  byId("db-equity-expand").addEventListener("click", () => {
    const el = byId("equity-chart");
    ...
    const fallbackExpand = () => {
      // Fullscreen API blocked/unsupported (e.g. some mobile browsers) — fall back
      // to the enlarged-card view so the button still does something useful.
      el.classList.toggle("db-chart-expanded");
      if (state.equityChart) setTimeout(() => state.equityChart.resize(), 0);
    };
    if (request) {
      try {
        const result = request.call(el);
        if (result && typeof result.catch === "function") result.catch(fallbackExpand);
      } catch (err) {
        fallbackExpand();
      }
    } else {
