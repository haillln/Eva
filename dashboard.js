/* =====================================================================
   ...
   Dashboard uses the shared journal account/trade path:
   users/{uid}/accounts/{accountId} and its trades subcollection.
   All UI calculations are derived from the selected account + its real trades.
   ===================================================================== */
(function () {
  function initDashboard(wrap) {
  ...
  function normalizeTrade(id, raw, accountId) {
    ...
    let result = (raw.result || '').toString().trim().toLowerCase();
    if (['win', 'profit', 'profitable', 'winner', 'won'].includes(result)) result = 'win';
    else if (['loss', 'lose', 'losing', 'loser', 'lost'].includes(result)) result = 'loss';
    else if (['breakeven', 'break-even', 'break_even', 'be', 'even', 'flat'].includes(result)) result = 'be';
    else if (!result) {
  ...
  function subscribeAccounts(uid) {
    ...
    state.unsubscribeAccounts = fs.onSnapshot(
      ...
      (snap) => {
        ...
        const stillExists = state.selectedAccountId && state.accounts.some((a) => a.id === state.selectedAccountId);
        if (!stillExists) {
  ...
  function selectAccount(accountId) {
    ...
    renderAccountHeader();
    if (el.filterAccount) el.filterAccount.value = accountId;
  ...
  function getStartingBalance(acc) {
    const candidates = [acc?.startingBalance, acc?.startBalance, acc?.initialBalance, acc?.accountBalance, acc?.balance];
  ...
  function renderAccountHeader() {
    ...
    if (el.acctBalance && acc) el.acctBalance.textContent = `Starting ${fmtMoney(getStartingBalance(acc))}`;
  }
  ...
  function setAccountPanel(open) {
    ...
    if (!el.acctPanel || !el.acctTrigger) return;
    if (!open) {
      el.acctPanel.style.top = '';
      el.acctPanel.style.left = '';
      el.acctPanel.style.right = '';
      el.acctPanel.style.transform = '';
      el.acctPanel.style.position = '';
      el.acctPanel.style.maxHeight = '';
      return;
    }
    if (window.matchMedia('(max-width: 640px)').matches) {
      const rect = el.acctTrigger.getBoundingClientRect();
      const margin = 12;
      const panelWidth = Math.min(window.innerWidth - margin * 2, 320);
      const top = Math.max(8, rect.bottom + 8);
      const available = Math.max(180, window.innerHeight - top - 12);
      el.acctPanel.style.position = 'fixed';
      el.acctPanel.style.left = '50%';
      el.acctPanel.style.right = 'auto';
      el.acctPanel.style.transform = 'translateX(-50%)';
      el.acctPanel.style.top = `${top}px`;
      el.acctPanel.style.width = `${panelWidth}px`;
      el.acctPanel.style.maxWidth = `calc(100vw - ${margin * 2}px)`;
      el.acctPanel.style.maxHeight = `${available}px`;
      el.acctPanel.style.zIndex = '2000';
    }
  }
  ...
  function ensureCustomRangeModal() {
    ...
    document.getElementById('custom-range-apply').addEventListener('click', () => {
      ...
      const end = new Date(`${e2}T23:59:59.999`);
  ...
  /* ======================= 7. CARDS RENDER =========================== */
  function renderCards(m, accountMetrics = m) {
    // Account balance/equity are always based on the complete selected-account
    // trade history. Filters only affect performance statistics/charts.
    setVal('balance', fmtMoney(accountMetrics.balance));
    setVal('total-pl', fmtMoney(m.totalPL), plClass(m.totalPL));
    setVal('equity-card', fmtMoney(accountMetrics.balance));
    setVal('equity', fmtMoney(accountMetrics.balance));
  ...
  function renderChallengeChart(m) {
    ...
    if (!chart) return;
    const acc = state.accounts.find((a) => a.id === state.selectedAccountId);
    const target = acc?.rules?.target ?? acc?.profitTarget ?? acc?.targetProfit ?? acc?.goal ?? acc?.profitGoal;
  ...
  document.querySelectorAll('[data-chart-action]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const chart = charts[btn.getAttribute('data-chart-id')];
      if (!chart) return;
      ...
      if (action === 'download') {
        ...
        a.href = url;
        a.download = 'eva-equity-chart.png';
        a.click();
      } else if (action === 'expand') {
        const container = chart._container || chart.getDom().parentElement;
  ...
  function rebuildDerivedAndRender() {
    ...
    const acc = state.accounts.find((a) => a.id === state.selectedAccountId);
    const startingBalance = acc ? getStartingBalance(acc) : 0;
    ...
    // Preserve full-account balance/equity while allowing filters to affect
    // performance statistics and charts.
    m.accountBalance = accountMetrics.balance;
    m.accountMetrics = accountMetrics;
    state.dashboardMetrics = m;
    renderCards(m, accountMetrics);
    renderAllCharts(m);
    renderCalendar();
  ...
  function waitForDashboard() {
    const dashboard = document.getElementById('eva-dashboard');
    if (dashboard && !dashboard.dataset.dashboardInitialized) {
  ...
  if (!waitForDashboard()) {
    const observer = new MutationObserver(() => {
      if (waitForDashboard()) observer.disconnect();
    });
