/* =====================================================================
   ...
   =====================================================================
   REAL FIRESTORE SCHEMA (verified against accounts.html, journal.js,
   challenge.js — journal and challenge accounts share ONE collection):
     users/{uid}/accounts/{accountId}
       name, accountType ('Live'|'Demo'|'Prop Firm' for journal;
         free-text program name for challenge accounts), isActive,
       initialBalance, currentBalance, broker, currency, createdAt
       -- Challenge-only accounts additionally carry accountSize,
          currentPhase, phasesEnabled, phaseRules, status. Their
          presence is how this file tells a challenge account apart
          from a journal account (see isJournalAccount() below).
     users/{uid}/accounts/{accountId}/trades/{tradeId}
       instrument, direction, tradeDate, entryTime, exitTime,
       entryPrice, exitPrice, stopLoss, takeProfit, lotSize, risk,
       riskReward, profitLoss, result ('Win'|'Loss'|'BE'), session,
       strategy, setup, notes, rating

   normalizeTrade()/isJournalAccount() below are the ONLY places that
   translate real field names into the shape the rest of this file
   already renders from — everything downstream is unchanged.
   ===================================================================== */
(function () {
  ...
  function normalizeTrade(id, raw, accountId) {
    // P/L: journal.js/challenge.js always write `profitLoss`. Older
    // fallbacks kept for safety, but profitLoss is the real field.
    let pnl = raw.profitLoss ?? raw.pnl ?? raw.pl ?? raw.profit ?? 0;
    pnl = Number(pnl) || 0;

    // Real result values are 'Win' | 'Loss' | 'BE' (journal.js normalizes
    // "Break Even" -> "BE" before it ever reaches Firestore).
    ...
    // journal.js/challenge.js store the trade date as `tradeDate`
    // (Firestore Timestamp on read). closeTime/date/openTime kept only
    // as fallbacks for any legacy docs that predate that field.
    const closeMs = toMillis(raw.tradeDate) ?? toMillis(raw.closeTime) ?? toMillis(raw.date) ?? toMillis(raw.openTime);

    // riskReward is free-text (e.g. "1:2.5"); only use it as the R
    // multiple when it actually parses to a plain number.
    const parsedRR = raw.riskReward != null && raw.riskReward !== '' && !Number.isNaN(Number(raw.riskReward))
      ? Number(raw.riskReward) : null;

    return {
      id,
      accountId,
      symbol: raw.instrument ?? raw.symbol ?? '—',
      direction: raw.direction ?? '—',
      entry: raw.entryPrice ?? raw.entry ?? null,
      exit: raw.exitPrice ?? raw.exit ?? null,
      ...
      rMultiple: raw.rMultiple ?? raw.R ?? parsedRR,
      openTimeMs: toMillis(raw.tradeDate) ?? toMillis(raw.openTime),
      closeTimeMs: closeMs,
      session: raw.session ?? '',
  ...
  /* Journal vs challenge accounts share ONE Firestore collection
     (users/{uid}/accounts) — confirmed from accounts.html (writes
     accountType: 'Live'|'Demo'|'Prop Firm') and challenge.js (writes
     accountType + accountSize + currentPhase + phasesEnabled +
     phaseRules for every challenge account it creates). A journal
     account never has those challenge-only fields, so their absence
     is the reliable discriminator — this is what keeps challenge/funded
     accounts out of the journal Dashboard's account selector. */
  function isJournalAccount(a) {
    return a.phasesEnabled === undefined
      && a.currentPhase === undefined
      && a.accountSize === undefined
      && a.phaseRules === undefined;
  }
  ...
  function normalizeAccount(id, raw) {
    return {
      ...raw,
      id,
      name: raw.name || raw.accountName || 'Account',
      // Real field is initialBalance; keep old fallbacks for legacy docs.
      startingBalance: Number(raw.initialBalance ?? raw.startingBalance ?? raw.balance) || 0,
      currentBalance: raw.currentBalance,
      status: raw.accountType || raw.status || 'Journal',
    };
  }
  ...
  function subscribeAccounts(uid) {
    ...
    state.unsubscribeAccounts = fs.onSnapshot(
      ref,
      (snap) => {
        // Only journal accounts belong on this dashboard — challenge/funded
        // accounts (same collection) are filtered out here.
        state.accounts = snap.docs
          .map((d) => normalizeAccount(d.id, d.data()))
          .filter(isJournalAccount);
        renderAccountSelector();
        const stillExists = state.selectedAccountId &&
          state.accounts.some((a) => a.id === state.selectedAccountId);
        if (!stillExists && state.accounts.length) {
